...
I chose Galveston because it has 146 years of data, which is a lot!  It's also coastal and higly vulnerable, so we might be able to see interesting data from big tropical cyclones.  There is a risk that big TC's become outliers that make GEV harder, but it could still be an interesting case study.  

**Response 2:** Fitting Methods Comparison  

Extremes MLE and Extremes PWM are almost identical, which makes sense, because these are both GEV MLE methods.  Extremes PWM branchse out a little further, with a shape parameter that shifts it out a little bit more to the right, as if it's trying harder to incorporate an outlier data point than the other two methods.  

MLE maximizes the product of the probability density function for each data point, so it makes sense that these functions would gravitate closer towards the norm for most of the non-outliers. 

**Response 3:** Multi-Station Geographic Analysis  

The red line, which is the second highest in terms of return level, is the closest to our original station and is still on the island of Galveston.  It makes sense that this station would be one of the highest, due to its vulnerable position on the coast, although it's interesting that it's surpassed by an inland station at Armand Bayou and Genoa Red Road (Purple Line).

To be fair, this location is still relatively close to the coast and on the edge of Galveston Bay, so it seeing a lot of rainfall isn't out of the question. 

**Response 4:** Uncertainty and Sample Size Insights  

Even with 100 years of data, we still have a decent amount of uncertainty in our 250 year return period, from 10 inches all the way to 20 inches.  If building infrastructure to a 250 year standard, I would pursue a sample size of 100 years or larger and set a large safety factor.  

This experiment shows that the variance amongst the real stations might be more due to randomness in storm arrival than it was due to geography, as I originally believed.  

This is a lower bound for the uncertainty in real-world applications, because we still have many stations out there for which we have not yet collected 100 years of data.  

**Response 5:** Final Synthesis 

When designing infrastructure in response to climate risk, we need to accurately assess risk, and then base large safety margins off of that assessment.  As important as it is to ask if we can afford to build a piece of infrastructure to a certain standard, we should also ask if we can afford to have that piece of infrastructure fail. 
